	...
	<view class="container">
		...
		<view class="code-box">
			<view v-for="(item, index) in codeList" :key="index"
				:class="['code-item', { active: currentIndex === index }]" @click="setFocus(index)">
				...
				<view v-if="currentIndex === index && !item" class="cursor"></view>
			</view>
		</view>
		...
		<view class="keyboard">
			...
			<view class="row">
				<view v-for="k in row4" :key="k.value" :class="['key', k.type]" @click="handleSpecial(k)">
			...
			<view class="row">
				<view v-for="k in row5" :key="k.value" :class="['key', k.type]" @click="handleSpecial(k)">
					<text v-if="k.type === 'normal'">{{ k.value }}</text>
